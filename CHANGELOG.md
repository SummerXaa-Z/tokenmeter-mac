...
## v3.9.7 — 2026-09-25 — 滚动 7 天环比与热力图连续天数

- **环比卡「近 7 天」滚动档**：周期环比卡新增第三档 近 7 天 vs 前 7 天——两个不重叠的滚动窗口（本期 = 今天往前 7 天、上期 = 再往前 7 天），周初看日历周环比会因本周才过一两天而虚低，滚动口径反映真实势头（周末的大用量也不会被日历周边界切掉）；区间终点为排他边界，跨月也正确。影响范围：`PeriodCompare.swift`、`OverviewCards.swift`、`PeriodCompareTests.swift`。
- **热力图连续天数与今天标记**：热力图脚注新增深浅图例（少→多四档色块）与「当前连续 N 天」——连续口径与个人画像完全一致（从今天倒着数，今天尚未开始使用时容忍一次空白、从昨天起算，再遇空白即断，平台账户不计）；今天的格子加描边环，一眼定位"今天到哪了"。影响范围：`UsageHeatmap.swift`、`OverviewCards.swift`、`UsageHeatmapTests.swift`。
